...
@Component({
  selector: 'app-dasboard-venta-fc',
  templateUrl: './dasboard-venta-fc.component.html',
  styles: [`
...
export class DasboardVentaFcComponent implements OnInit {
  ...
  setData(){
    ...
    for( let r of this.dataExample ){
      ...
      series['fcReal'].push(Math.round(r['fcReal'] * 100) / 100)
      series['fcDia'].push(Math.round(r['fcDia'] * 100) / 100)
    }
  ...
  getData( ft = false, a? ) {
    ...
    this._api.restfulGet( a ? a : this.asesor, 'Dashboard/ventaFc' )
                .subscribe( res => {
                  ...
                  if( ft || !this.chart ){
                    this.chart = Highcharts.chart('containerFc', this.options);
                  }
